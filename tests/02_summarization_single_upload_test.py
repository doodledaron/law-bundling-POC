...
# Load .env from parent directory (since we run from tests/)
load_dotenv('../.env')
...
def test_single_upload():
    ...
    # API base URL - Use API_BASE_URL from .env or construct from API_PORT
    ...
    print(f"ℹ️  Using API Base URL: {api_base_url}")
...
%%EOF"""
    ...
    try:
        ...
        with open(test_file_path, 'rb') as f:
            ...
            response = requests.post(f"{api_base_url}/api/upload", files=files, headers=headers)
        ...
        max_attempts = 240  # 60 minutes max (PPStructure can be slow)
        attempt = 0
        
        while attempt < max_attempts:
            response = requests.get(f"{api_base_url}/api/job/{job_id}", headers=headers)
