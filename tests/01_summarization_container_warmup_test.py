...
def check_job_status(job_id, test_num=1, max_wait_seconds=300):
...
def main():
    ...
    if jobs:
        ...
        # Monitor all jobs with longer timeout for PPStructure processing
        for job_id, test_num in jobs:
            success = check_job_status(job_id, test_num=test_num, max_wait_seconds=600)
            if success:
                completed_jobs += 1
            else:
                failed_jobs += 1
