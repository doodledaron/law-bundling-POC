...
def ensure_pipeline_initialized():
    ...
    try:
        ...
        # Try high-performance inference first, with graceful fallback
        try:
            logger.info("🚀 Attempting to initialize PPStructure with high-performance inference...")
            pipeline = create_pipeline(
                pipeline="PP-StructureV3",
                use_hpip=True
            )
            logger.info("✅ High-performance inference pipeline initialized successfully!")
            ...
            try:
                ...
                logger.error(f"❌ Standard pipeline also failed: {std_error}")
                raise RuntimeError(f"All pipeline initialization attempts failed: {std_error}")
        ...
        for page_index, (image_path, output) in enumerate(zip(image_paths, all_outputs)):
            ...
            if 'overall_ocr_res' in output_dict and 'rec_texts' in output_dict['overall_ocr_res']:
                ...
                all_ocr_text.extend(ocr_text)
                    ...
                    # Process tables, figures, and charts with Gemini AI (conditional)
        chart_extractions = []
...
# Alternative configuration for fine-tuned control (add this as a comment for reference)
"""
Alternative high-performance configuration for fine-tuned control:
    ...
    hpi_config={
        ...
        "SubModules": {
            "TextDetection": {
                ...
                "backend_config": {
                    "dynamic_shapes": {
                        "x": [[1, 3, 224, 224], [1, 3, 1024, 1024], [1, 3, 2048, 2048]]
                    }
                }
            },
            "TextRecognition": {
                "backend": "onnxruntime"  # Sometimes ONNX is better for text recognition
            },
            "LayoutDetection": {
                "backend": "tensorrt"  # TensorRT excellent for layout detection
            }
        }
    }
)
...
def ensure_pipeline_initialized():
    ...
    try:
        ...
        # Initialize with high-performance inference plugin
        # This uses TensorRT for optimal GPU performance on your CUDA setup
        try:
            logger.info("🚀 Attempting to initialize PPStructure with TensorRT high-performance inference...")
            pipeline = create_pipeline(
                pipeline="PP-StructureV3",
                use_hpip=True,
                hpi_config={
                    "auto_config": True,  # Safe auto-configuration mode
                    "backend": "tensorrt",  # Best for NVIDIA GPUs
                    "backend_config": {
                        "dynamic_shapes": {
                            "x": [
                                [1, 3, 224, 224],    # Min document size
                                [1, 3, 1024, 1024],  # Typical document size  
                                [1, 3, 2048, 2048]   # Max document size
                            ]
                        }
                    }
                }
            )
            logger.info("✅ High-performance TensorRT pipeline initialized successfully!")
            
        except Exception as tensorrt_error:
            logger.warning(f"⚠️ TensorRT initialization failed: {tensorrt_error}")
            ...
            try:
                pipeline = create_pipeline(
                    ...
                    hpi_config={
                        "auto_config": True,
                        "backend": "onnxruntime"  # Fallback backend
                    }
                )
                logger.info("✅ ONNX Runtime pipeline initialized as fallback!")
                
            except Exception as onnx_error:
                logger.warning(f"⚠️ ONNX Runtime fallback failed: {onnx_error}")
                logger.info("🔄 Falling back to standard pipeline without high-performance inference...")
                ...
                logger.info("✅ Standard pipeline initialized (no high-performance inference)")
